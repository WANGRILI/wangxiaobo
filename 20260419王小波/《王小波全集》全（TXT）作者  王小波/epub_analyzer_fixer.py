...
class EPUBAnalyzerAndFixer:
    ...
    def _load_with_ebooklib(self, epub_path, analysis):
        ...
        try:
            ...
            analysis['metadata']['title'] = title[0] if title else '未知'
            ...
            if book.get_metadata('DC', 'description'):
                description = book.get_metadata('DC', 'description')[0]
                analysis['metadata']['description'] = description[:100] + '...' if len(description) > 100 else description
            ...
            with ZipFile(epub_path, 'r') as zip_file:
                ...
                if opf_files:
                    ...
                    try:
                        ...
                        # 简单的元数据提取
                        title_match = re.search(r'<dc:title>(.*?)</dc:title>', opf_str, re.IGNORECASE)
                        if title_match:
                        ...
                        if creator_match:
                            analysis['metadata']['creators'] = [creator_match.group(1).strip()]
                            print(f"  ✓ 作者: {creator_match.group(1).strip()}")

                    except Exception as e:
                        analysis['warnings'].append(f"OPF解析问题: {str(e)}")
            ...
            with ZipFile(epub_path, 'r') as zip_file:
                ...
                for opf_file in opf_files:
                    try:
                        ...
                        analysis['warnings'].append(f"OPF文件检查错误: {str(e)}")
            ...
            with ZipFile(epub_path, 'r') as zip_file:
                ...
                for ncx_file in ncx_files:
                    try:
                        ...
                        analysis['warnings'].append(f"NCX检查错误: {str(e)}")
            ...
            with ZipFile(epub_path, 'r') as zip_file:
                ...
                for html_file in html_files[:3]:  # 只检查前3个文件
                    try:
                        ...
                        analysis['warnings'].append(f"HTML文件检查错误: {str(e)}")
    ...
    def _check_resources(self, epub_path, analysis):
        ...
        try:
            ...
            analysis['warnings'].append(f"资源检查失败: {str(e)}")
    ...
    def _fix_with_repack(self, epub, analysis):
        """通过重新打包修复"""
        try:
            # 创建新的EPUB
            book = epub.EpubBook()
